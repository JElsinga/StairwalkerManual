...
The layer which represents the dataset with a style created in \Fref{sec:addingstyle} has now been created and is ready for use. A layer can be edited once it has been created so if changes need to be made a new layer should not be created. \Fref{sec:previewlayer} shows how to preview a layer and \Fref{sec:clientsidedev} discusses how to use a GeoServer layer with OpenLayers\footnote{\url{http://openlayers.org/}} to a geospacial visualization of the data on a web page.