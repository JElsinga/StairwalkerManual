...
GeoServer should already be deployed on a web server (see \Fref{sec:geotomcat}), and can then be accessed with \lstinline|http://<Tomcat-IPaddress>:8080/geoserver/web/|. It is required to login in order to work on the server side of GeoServer. When using the default setup of GeoServer the login credentials are: \\
